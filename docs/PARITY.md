...
| `ru_allow_force_ready 0\|1` | `ConfigConvars.cs` | none | missing | Gates `.forceready`. | none | S |
...
| `match_paused` / `unpause_requested` / `match_unpaused` | same | same, but `is_tactical` is always false and `pause_time` 0 | partial | Fill in `is_tactical` and `pause_time` once there are tactical/tech pauses (§7). | S |
...
| `.forceready` | `ReadyLogic.cs` | none | missing | Admin marks a team ready (`ru_allow_force_ready`). | none | S | P1 |
| `.start/.forcestart`, `.restart/.rr`, `.endmatch/.forceend` | `ConsoleCommands.cs` | `.ru start/restart/end` | partial | Chat aliases | none | S | P2 |
| `.pause/.p` tactical vs `.tech` technical; limits `ru_max_pauses_per_team`, `ru_pause_duration`, `ru_both_teams_unpause_required` (match cvars) | `Pausing.cs`, `ConfigConvars.cs:128-130` | `.pause` = `.tech` = `mp_pause_match`, no limits, both teams must unpause (RU: `ru_router.cpp`, `pause_state.cpp`) | partial | Pause types, per-team count, max duration with auto-unpause, the one-team-unpause option, and `is_tactical` in events | none | M | P1 |
| `.tac` tactical timeout | `ConsoleCommands.cs:399` | none (`mp_team_timeout_*` are already in `cfg/ReadyUp/live.cfg`) | missing | Call `timeout_ct_start` / `timeout_terrorist_start` for the caller's side | none (built-in commands) | S | P1 |
| `.forcepause/.fp`, `.forceunpause/.fup` | `ConsoleCommands.cs:278` | `.ru fp/fup` | partial | Chat aliases. An admin pause is only unpausable by an admin. | none | S | P2 |
...
| Forfeit when a team leaves (`ru_ffw_enabled`, `ru_ffw_time`) | `ConfigConvars.cs:143` | none (`.ff` captain forfeit emits `match_forfeit`, which the platform ignores) | missing | Timer while a whole team is disconnected → forfeit win. Pick `.ff` or ffw and send the AT result events. | none | S | P1 |
...
| Team names in game (`mp_teamname_1/2`, flags) | `MatchManagement.cs` | **not set** | missing | Set `mp_teamname_1/2` (and `mp_teamflag_*`) on load and after swaps | none (cvars) | S | P1 |
